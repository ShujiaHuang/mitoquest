...
std::string vcf_header_define(const std::string &ref_file_path, const std::vector<std::string> &samples) {
    std::vector<std::string> header = {
        ...
        "##FORMAT=<ID=SB,Number=1,Type=String,Description=\"An ordered, '|' delimited list of read number of mapping strand orientation for ref and alt alleles in the order listed: ref_fwd,ref_rev,alt_fwd,alt_rev|...\">",
        "##FORMAT=<ID=FS,Number=1,Type=String,Description=\"An ordered, '|' delimited list of phred-scaled p-value using Fisher's exact test to detect strand bias\">",
        "##FORMAT=<ID=SOR,Number=1,Type=String,Description=\"An ordered, '|' delimited list of strand bias estimated by the Symmetric Odds Ratio test\">",
