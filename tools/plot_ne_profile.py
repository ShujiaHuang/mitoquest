#!/usr/bin/env python3
"""Plot the deCODE-style Ne-profile comparison between MLE and Kimura.
...
scores every candidate Ne under both estimators in the program:
    ...
    mle_log_lik(Ne)  -- global log-likelihood under the configured model
                        (continuous Beta-diffusion or discrete Beta-Binomial).
                        Maximised at the fitted Ne_MLE.
...
The output figure has two panels:
    ...
    Left   --  -2(LL - LL_max) vs Ne, with the chi^2 95% CI threshold (3.84).
               The fitted Ne_MLE and its 95% profile CI are annotated.
...
def load_ne_profile(tsv_path: str):
    ...
    Drops rows where mle_log_lik is non-finite (Ne = 1 under the
    ...
    df = df[np.isfinite(df["mle_log_lik"])].reset_index(drop=True)
    return meta, df
...
def plot_mle_panel(ax, df: pd.DataFrame, meta: dict) -> None:
    """Left panel: -2(LL - LL_max) vs Ne under the MLE."""
    ax.plot(df["ne_candidate"], df["mle_delta_2ll"],
            color="#c0392b", linewidth=2.0, label="MLE  -2 (LL - LL_max)")
    ...
    ne_mle    = _meta_float(meta, "fitted_ne_mle")
    ne_lo     = _meta_float(meta, "fitted_ne_mle_ci_low")
    ne_hi     = _meta_float(meta, "fitted_ne_mle_ci_high")
    if np.isfinite(ne_mle):
        ax.axvline(ne_mle, color="#c0392b", linestyle="-", linewidth=1.5,
                   label=f"Fitted Ne_MLE = {ne_mle:.2f}")
...
def make_plot(tsv_path: str, output_path: str,
    ...
    if df.empty:
        raise RuntimeError(
            "[plot_ne_profile] No usable rows in profile (all mle_log_lik are -inf?)")

    fig, (ax_left, ax_right) = plt.subplots(1, 2, figsize=figsize)
    plot_mle_panel(ax_left, df, meta)
    ...
    fig.text(0.5, 0.005,
             ...
             f"Ne_MLE = {meta.get('fitted_ne_mle', '?')},  "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot the MLE vs Kimura Ne-profile diagnostic emitted by "
