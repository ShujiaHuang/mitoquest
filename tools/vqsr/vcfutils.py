...
class Header(object):
    ...
    def __init__(self, hInfo = None): 
        ...
        if hInfo and (type(hInfo) is not dict): 
            raise ValueError (f'The data type should be "dict" in class of '
                              f'"VCFHeader", but found {str(type(hInfo))}')
        if hInfo:
            self.header = hInfo
...
class Info(object): 
    def __init__(self, info = None):
        ...
        if info and (type(info) is not dict): 
            raise ValueError (f'The data type should be "dict" in class '
                              f'of "VCFInfo", but found {str(type(info))}')
...
def loadPedigree(pedigree_file):
    ...
    for line in open(pedigree_file):
        ...
        if col[1] in pedigree:
            raise ValueError(f'[ERROR] {col[1]} is already in "pedigree". Your file ' 
                             f'may have the duplication sample name.')
