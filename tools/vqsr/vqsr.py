...
def main(opt):
    ...
    annoTexts = [d[0] for d in vr.data_manger.anno_texts]
    ...
    with gzip.open(opt.vcf_infile, "rt") if opt.vcf_infile.endswith(".gz") else open(opt.vcf_infile, "r") as IN:
        for line in IN:
            n += 1
            if n % 100000 == 0:
