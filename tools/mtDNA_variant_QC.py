...
def iterative_beta_fit_and_call(
    ...
    for r in results:
        ...
        r['posterior'] = np.max(new_pps) if new_pps else 0  # For multi-allelic sites, take the maximum posterior across all ALTs as 
                                                            # the final posterior for the site. This is a more conservative approach 
                                                            # than taking the product, as it assumes that the evidence from each ALT 
                                                            # is not independent and that the strongest evidence should dominate the 
                                                            # mutation call.
        r['is_mutation'] = r['posterior'] > threshold  # Update mutation call based on new posterior
        r['new_gt'] = [g if (g is not None and pp > threshold) else None 
                       for pp, g in zip(new_pps, r['gt'])]  # Update GT to only include alleles that are called as mutations based on 
                                                            # the new posterior (optional, can keep original GT for reference)
